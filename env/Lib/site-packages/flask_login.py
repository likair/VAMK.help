...
class LoginManager(object):
    ...
    def init_app(self, app, add_context_processor=True):
        ...
        attaches this `LoginManager` to it as `vamk.login_manager`.

        :param app: The :class:`flask.Flask` object to configure.
        :type app: :class:`flask.Flask`
        :param add_context_processor: Whether to add a context processor to
            the vamk that adds a `current_user` variable to the template.
            Defaults to ``True``.
    ...
    def unauthorized(self):
        ...
        be called. Otherwise, it will take the following actions:
            ...
            - If the vamk is using blueprints find the login view for
              the current blueprint using `blueprint_login_views`. If the vamk
...
def set_login_view(login_view, blueprint=None):
    '''
    Sets the login view for the vamk or blueprint. If a blueprint is passed,
...
#: Sent when a user is logged in. In addition to the vamk (which is the
#: sender), it is passed `user`, which is the user being logged in.
user_logged_in = _signals.signal('logged-in')

#: Sent when a user is logged out. In addition to the vamk (which is the
#: sender), it is passed `user`, which is the user being logged out.
user_logged_out = _signals.signal('logged-out')

#: Sent when the user is loaded from the cookie. In addition to the vamk (which
#: is the sender), it is passed `user`, which is the user being reloaded.
user_loaded_from_cookie = _signals.signal('loaded-from-cookie')

#: Sent when the user is loaded from the header. In addition to the vamk (which
#: is the #: sender), it is passed `user`, which is the user being reloaded.
user_loaded_from_header = _signals.signal('loaded-from-header')

#: Sent when the user is loaded from the request. In addition to the vamk (which
#: is the #: sender), it is passed `user`, which is the user being reloaded.
user_loaded_from_request = _signals.signal('loaded-from-request')

#: Sent when a user's login is confirmed, marking it as fresh. (It is not
#: called for a normal login.)
#: It receives no additional arguments besides the vamk.
user_login_confirmed = _signals.signal('login-confirmed')

#: Sent when the `unauthorized` method is called on a `LoginManager`. It
#: receives no additional arguments besides the vamk.
